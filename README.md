...
For convenience, the local development server proxies `/api` requests to the live RealWorld-compatible API server at https://conduit-api.bondaracademy.com/api. This avoids browser CORS failures during local development. The original RealWorld demo API at https://api.realworld.io/api is no longer reliably available. You can view [the API spec here](https://github.com/GoThinkster/productionready/blob/master/api) which contains all routes & responses for the server.
